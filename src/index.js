const { BaseKonnector, saveBills, log, utils } = require('cozy-konnector-libs')
...
async function start(fields, cozyParameters) {
  ...
  if (await login.bind(this)(fields.login, fields.password)) {
    ...
    try {
      ...
      if (await logout.bind(this)()) log('info', 'Successfully logged out')
      else log('error', 'Loggin out failed. Exiting.')
    }
  } else {
...
async function getInstallations() {
  ...
  if (response.ok && jsonData.errors == undefined) {
    log(
      'info',
      'Found ' + xSInstallations.installations.lenght + ' installations.'
    )
    for (let i in xSInstallations.installations) {
      let installation = xSInstallations.installations[i]
      log(
        'debug',
        'Installation ' + installation.numinst + ' ' + installation.alias
      )
...
async function getInvoices(numinst) {
  ...
  let raw = JSON.stringify({
    ...
    query:
      'query mkInvoiceList($numinst: String!, $type: BillType) {\n  xSInvoiceList(numinst: $numinst, billType: $type) {\n    config {\n      address\n      city\n      owner\n      postcode\n      province\n    }\n    invoices {\n      numInvoice\n      contract\n      date\n      total\n    }\n  }\n}\n'
  })
...
async function parseInvoices(installation, invoices) {
  ...
  for (let i in invoices) {
    ...
    let bill = {
      ...
      filename: `${utils.formatDate(invDate)}_${VENDOR}_${amount.toFixed(
        2
      )}${cur}_${installation}_${invoice.numInvoice}.pdf`,
      vendor: VENDOR,
      type: 'Security',
      subtype: 'Alarm',
      metadata: {
